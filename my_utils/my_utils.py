...
reference_image_folder = "/root/comfy/ComfyUI/output/refer"
    ...
    try:
        with open(file_path, "r") as file:
            for line in file:
                ...
                _, name, _1, _2 = parts[:4]  # Exclude the first part (rare token)
                names.append(name)
    except FileNotFoundError:
...
def populate_dropdowns():
    ...
    if (os.path.getsize(database_file) == 0):
        return [gr.update(choices=["None"], value="None") for _ in range(4)]
    character_names = extract_names_without_rare_tokens(database_file)
    return [gr.update(choices=["None"] + character_names, value=character_names[0]) for _ in range(4)]
...
def get_character_name(selected_image_path, file_path="database.txt"):
    ...
    with open(file_path, "r") as file:
        ...
        # Check if the input string exists in the file
        for line in lines:
            if selected_image_path.strip() == line.split(";")[2].strip():
                return line.split(";")[1].strip()  # Found the string
...
def adapt_portrait(portrait_type, image_path, output_path, is_control_image=False, portrait_portion=[292, 219]):
    upscale_filter = Image.Resampling.LANCZOS
    pillow_legacy_filter = Image.LANCZOS
    ...
    with Image.open(image_path) as img:
        ...
        if portrait_type == "full body portrait":
            ...
            if portrait_portion == [354, 457]:
                width, height = width1, height1
            else:
                width, height = width2, height2
            # Save the cropped image
            final_img = cropped_img.resize((width, height), resample=pillow_legacy_filter)
...
def modify_character(regenerate_character_name, image_path, character_prompt, database_file="database.txt"):
    ...
    # updated_lines = []
    # for line in lines:
    #     if regenerate_character_name == line.split(";")[1].strip():
    ...
    with open(database_file, "w") as f:
        for line in lines:
            ...
            if not line:  # 跳过空行
                continue
            parts = line.split(";")
            # 检查字段数量是否足够（至少两个分号分隔的字段）
            if len(parts) >= 2 and regenerate_character_name == parts[1].strip():
                # 匹配成功，写入新行
                new_line_content = (";".join(line.strip().split(";")[:2] + [image_path] + [character_prompt]) + "\n") 
...
def draw_text_bubble_on_image(
    ...
    # ----------------------------------------------------------------------
    font_path = "/workspace/fonts/font2.ttf"  # Update with your actual font path
    font_size = 34
    min_font_size = 10
    padding = 10
...
def save_pdf(pathA="/root/reference_image0.png", 
             pathB="/root/reference_image1.png", 
             pathC="/root/reference_image2.png", 
             pathD="/root/reference_image3.png", 
             output_pdf="layout_2x2.pdf", border_size=2, border_color=(0, 0, 0), padding=10):
    """
    ...
    """
    # last_4_images = os.listdir(reference_image_folder)[-4:]
    # pathA = os.path.join(reference_image_folder, last_4_images[0])
    # pathB = os.path.join(reference_image_folder, last_4_images[1])
    # pathC = os.path.join(reference_image_folder, last_4_images[2])
    # pathD = os.path.join(reference_image_folder, last_4_images[3])
    ...
    combined.paste(imgC_resized, (2 * padding + wA, padding))
    combined.paste(imgB_resized, (padding, 2 * padding + hA))
    combined.paste(imgD_resized, (2 * padding + wA, 2 * padding + hB))
    combined.save(output_pdf, "PDF")
    
    return output_pdf
    # return combined
def simplified_atomic_write(final_path, data):
    """
    Writes data using a temporary file and atomic rename (minimal version).

    Args:
        final_path (str): The desired final path for the file.
        data (bytes): The binary data to write.
    """
    target_dir = os.path.dirname(final_path) or '.' # Handle files in current dir
    # Ensure target dir exists (basic necessity)
    os.makedirs(target_dir, exist_ok=True)

    tmp_path = None # Initialize for potential partial creation cleanup (minimal)
    try:
        # 1. Create temp file descriptor to get a unique name, then close it
        #    'delete=False' is crucial so the file isn't removed on close.
        #    'dir=target_dir' ensures it's on the same filesystem for atomic rename.
        #    We use prefix/suffix for clarity, but could be omitted for *absolute* minimum.
        ...
        tmp_path = tf.name
        tf.close() # Close the initial handle immediately after getting the name

        # 2. Write data to the temporary file path
        #    'with' ensures the file is closed after writing.
        with open(tmp_path, 'wb') as f:
            f.write(data)

        # 3. Atomically rename the temporary file to the final name
        #    This is the core step ensuring atomicity for readers.
        os.replace(tmp_path, final_path)
        tmp_path = None # Mark rename as successful

    finally:
        # Minimal cleanup: remove tmp file if rename failed (e.g., due to permissions)
        if tmp_path and os.path.exists(tmp_path):
             try:
                 os.remove(tmp_path)
             except OSError:
                 pass # Ignore cleanup errors in this simplified version


def simplified_read(final_path):
    """
    Reads the file only if it exists with the final name (minimal version).

    Args:
        final_path (str): The path to the final file.

    Returns:
        bytes or None: The file content if read successfully, otherwise None.
    """
    # Reader ONLY looks for the final path. It will never see the .tmp file.
